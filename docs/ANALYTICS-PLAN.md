...
2. ~~**UTMs se pierden**~~ **RESUELTO** (y el diagnóstico original estaba sobredimensionado). Dos capas, hay que distinguirlas:
   - **PostHog nativo: nunca se perdió.** `cross_subdomain_cookie` viene en `true` por default (la librería solo excluye `herokuapp.com`/`vercel.app`/`netlify.app`), y en modo `persistence: "localStorage+cookie"` la cookie de dominio raíz lleva `distinct_id` + `$initial_person_info` — que es justamente donde viven referrer y campaign params iniciales. O sea `$initial_utm_*` y la continuidad anónimo→identificado cruzan el subdominio solas. Igual para `_fbp` (Meta) y `_gcl_aw` (Google Ads), que también se setean a nivel dominio registrable.
   - **El store propio `loca-utm` sí se perdía**, porque `localStorage` es por origen: en `app.heyloca.ai` el `getFirstTouchUtm()` devolvía `null` y los `$set_once` custom de `identifyUser()` quedaban vacíos (props que, de todos modos, duplicaban los `$initial_utm_*` nativos). Resuelto por `<AppLinkUtms />` (`src/components/app-link-utms.tsx`): decora los links a la plataforma con los params de campaña después de montar, así `captureFirstTouchUtm()` los encuentra al otro lado. `appHref()` sigue sin tocarlos a propósito — se evalúa en el servidor y en module scope (`PLANS`), donde no hay `window`.
