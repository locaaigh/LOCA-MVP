...
/** Estado de la conexión de Instagram del negocio (sin tokens, vista segura). */
export async function GET(req: NextRequest) {
  ...
  try {
    const row = await getConnection(userId, businessId, "instagram");
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await deleteConnection(userId, businessId, "instagram");
    await logEvent({ userId, businessId, name: "instagram_disconnected" });
